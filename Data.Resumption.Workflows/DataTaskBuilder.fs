...
type DataTaskBuilder() =
    ...
    member inline __.Bind(task, cont) = bindTT task cont

    member inline __.Bind((a, b), cont) = bindTT (tuple2 a b) cont
    member inline __.Bind((a, b, c), cont) = bindTT (tuple3 a b c) cont
    member inline __.Bind((a, b, c, d), cont) = bindTT (tuple4 a b c d) cont
    ...
    member inline __.Using(disposable : #IDisposable, body) =
        let dispose () =
            ...
            | null -> ()
            | d -> d.Dispose()
        finallyTT (fun () -> body disposable) dispose

    member inline __.TryFinally(task, onExit) = finallyTT task onExit
    member inline __.TryWith(task, onExit) = catchTT task onExit

    member inline __.Combine(task, cont) = bindTT task (fun _ -> cont())
