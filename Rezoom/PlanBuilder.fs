...
type PlanBuilder() =
    ...
    member inline __.Delay(delayed : unit -> 'a Plan) : 'a Plan =
        { new Plan<'a>() with member __.Next() = (delayed ()).Next() }
    ...
    member inline __.Using(disposable : #IDisposable, body : #IDisposable -> 'a Plan) : 'a Plan =
        let dispose () =
            ...
            | null -> ()
            | d -> d.Dispose()
        tryFinally { new Plan<'a>() with member __.Next() = (body disposable).Next() } dispose
